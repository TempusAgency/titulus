...
rl5="$(printf '%s' "$INPUT" | jq -r '.rate_limits.five_hour.used_percentage // empty | if type=="number" then (floor|tostring) else "" end')"   # 5-hour rate-limit usage
...
printf '%b%s%b'   "${BLUE}${BOLD}" "$(wrapf "$W" "" "   " "● Назва: ${name}")" "$RESET"
[ -n "$goal" ] && printf '\n%b%s%b' "$YELLOW" "$(wrapf "$W" "" "   " "◎ Ціль: ${goal}")" "$RESET"
[ -n "$task" ] && printf '\n%s'     "$(wrapf "$W" "" "   " "▸ Задача: ${task}")"
[ -n "$prev" ] && printf '\n%b%s%b' "$DIM" "$(wrapf "$W" "" "   " "◃ Попередня: ${prev}")"
printf '\n%s' "$(rule "$W")"   # horizontal divider between the card and the meta bar
...
# subagents running? count agent-*.jsonl modified within the window. Portable stat (no -newermt,
# which flaked) + a 20s window (agents write in bursts; the line redraws only every few seconds,
# so a tight window gets missed). Lingers ~20s after they finish — acceptable.
...
# animated session icon: twinkles orange while a turn is running (marker set on submit, cleared
# on Stop); static ✻ otherwise. Frame follows wall-clock, so it moves as the line re-renders.
# "working" = explicit marker (new sessions with the UserPromptSubmit hook) OR the transcript was
# just written (works in ANY session, no hook needed — it's appended on each tool call/message).
working=0
...
[ -n "$rl5" ] && seg "${DIM}◴ 5г ${rl5}%${UNBOLD}" "◴ 5г ${rl5}%"
...
if [ "${agents_n:-0}" -gt 0 ]; then
  ...
  seg "${AGCOL}${BOLD}${aspin} агенти ×${agents_n}${UNBOLD}${FGDEF}" "${aspin} агенти ×${agents_n}"
fi

# One block PER working directory (cwd + every /add-dir'd dir from workspace.added_dirs):
# a folder row (⌂ name · branch) then its full-path row (↳ path) — so multiple roots read as
# clearly separate. ⌂ = U+2302; the folder name is an OSC-8 link that opens that path.
dirs=("$cwd")
